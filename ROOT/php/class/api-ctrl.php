    ...
    class API
    {
        ...
        public function recieve($req)
        {
            ...
            /*
                Request Looks like:
                {
                    ...
                    INFO REQ{
                        [req] "return" : "profile" (Thing to return. Can be of type: profile, profiles, search, events, event, lists, list, followers, following, reviews, reviewed, review, chats, chat)
            ...
            */
            ...
            if (empty($req) || !isset($req["type"]) || !isset($req["user_id"])) {
            ...
            } elseif ($req["type"] === "rate") {
                //RATE REQ
                if (!isset($req["username"]) || !isset($req["event_id"]) || !isset($req["r_rating"]) || !isset($req["r_name"]) || !isset($req["r_comment"]) || !isset($req["r_img"])) {
        ...
        */
        ...
        {
            if (isset($req["scope"]) && $req["scope"] === "global") {
                //Description: Get all lists
                //This is a multiple object request (not single object)
                $query = $this->conn->prepare('SELECT * FROM lists WHERE 1;');
                //error handling
                try {
                    $query->execute();
                    ...
                    if (!empty($lists)) {
                        //get lists
                        $this->respond("success", $lists, "Lists returned successfully");
                    } else {
                        $this->respond("error", null, $this->no_lists_err);
                    }
                    $query = null;
                } catch (PDOException $e) {
                    $this->respond("error", null,  $e->getMessage());
        ...
        public function getUsers($req)
        {
            ...
            try {
                ...
                if (!empty($users)) {
                    //get all users
                    $this->respond("success", $users, "Users returned successfully");
                } else {
                    $this->respond("error", null, "No users");
                }
                $query = null;
            } catch (PDOException $e) {
                $this->respond("error", null,  $e->getMessage());
